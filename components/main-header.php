<header class="header transparent-header">
  <div class="header__content">
    <div class="header__logo-container">
      <img src="./assets/png/uiet_header.png" class="header__logo-img" alt="UIET Logo" loading="lazy" />

      <span class="header__logo-sub"> </span>
    </div>
    <div class="header__main">
      <ul class="header__links">
        <li class="header__link-wrapper">
          <a href="#home" class="header__link--main"> Home </a>
        </li>
        <li class="header__link-wrapper" id="submenu-parent">
          <a href="#about" class="header__link--main">About</a>
          <ul class="submenu">
            <a href="./campus.php">Campus</a>
            <a href="https://youtu.be/rV5JhCceHgo">Campus Tour</a>
            <a href="./leadership.php">Leadership</a>
            <a href="./faculty.php">Faculty</a>
            <a href="./committees.php">Committees</a>
            <a href="./document.php">Documents</a>
          </ul>
        </li>
        <li class="header__link-wrapper" id="submenu-parent">
          <a href="#" class="header__link--main"> Academics </a>
          <ul class="submenu">
            <a href="./courses.php">Programs</a>
            <a href="./placements.php">Placements</a>
            <a href="https://unijammuopac.libsys10.in"> eLibrary </a>
            <a href="./results.php">Results</a>
          </ul>
        </li>

        <li class="header__link-wrapper">
          <a href="#admission" class="header__link--main"> Admission </a>
        </li>
        <li class="header__link-wrapper" id="submenu-parent">
          <a href="#" class="header__link--main">Help!</a>
          <ul class="submenu">
            <a href="https://forms.gle/bSRwEuV6sBcPyTRo6">uGRAMS</a>
            <a href="/contact.php">Contact us here!</a>
          </ul>
        </li>
      </ul>
      <div class="header__main-ham-menu-cont">
        <img src="./assets/svg/ham-menu.svg" alt="hamburger menu" class="header__main-ham-menu" />
        <img src="./assets/svg/ham-menu-close.svg" alt="hamburger menu close" class="header__main-ham-menu-close d-none" />
      </div>
    </div>
  </div>
  <div class="header__sm-menu">
    <div class="header__sm-menu-content">
      <ul class="header__sm-menu-links">
        <li class="header__sm-menu-link">
          <a href="#home"> Home </a>
        </li>

        <li class="header__sm-menu-link">
          <a href="#about"> About </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./courses.php">Programs</a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./amenities.php"> Campus </a>
          <a href="https://youtu.be/rV5JhCceHgo">Campus Tour</a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./leadership.php"> Leadership </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./faculty.php"> Faculty </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./committees.php"> Committees </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./placements.php"> Placements </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="#admission"> Admission </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./results.php"> Results </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="https://unijammuopac.libsys10.in"> eLibrary </a>
        </li>
        <li class="header__sm-menu-link">
          <a href="./document.php">Documents</a>
        </li>
        <li class="header__sm-menu-link">
          <a href="https://forms.gle/bSRwEuV6sBcPyTRo6">uGRAMS</a>
          <a href="/contact.php">Contact us here!</a>
        </li>
      </ul>
    </div>
  </div>
</header>